'''
Flask application to check the local user password and change it via API Calls for Aruba Clearpass
Tim Cappalli's API script is used at start:
...
# Import class cppm_connect from file (classes.py)
from classes import cppm_connect
from common_functions import change_password
#from common_functions import clearpass_api_test
#from common_functions import clearpass_postgres_test
...
def exp_days_f(cppm_class, current_user):
    ...
    uid, pwd_dt, attr, change_pwd_next_login = pg_sql('user', cppm_class, current_user)
...
def login():
    ...
    Login page function based on template (templates/login.html)
        ...
        Returns:
            1. Login page
            2. Rebuided Login page in case of an issue or incorrect credentials
               with status message
...
#    global status_code уже нет
    ...
    if request.method == 'POST':
        ...
        try:
            token_response, cppm_connect_main.status_code = get_access_token(cppm_connect_main, \
                session['username'], session['password'], get_access_type)
            # kill request.method
#            request.method = None
...
#   Start /login page
    ...
    return render_template('login.html', login_message=login_message, title='Log in')
...
def change_password_page():
    ...
    Asks user to input new password
        ...
        Returns:
            ...
            2. Rebuilded change password page in the case of an issue
               or entered incorrected password's pair (shows why incorrect)
            3. Redirect to logged in page (Function logged_in)
    '''
#    global status_code
#    print ('STATUS CODE change password' , status_code)
    if  cppm_connect_main.status_code != 200:
...
#            print (session['username'] , session['new_password1'])
#            print (status_code)
            ...
            if session['new_password1'] != session['new_password2']:
                ...
                return render_template('change_password_page.html', \
                    cp_message=cp_message, title='Change Password', \
                    username=session['username'])
            # Call change password function
#            elif patch_user_code != 200:
            try:
                patch_user_status, session['patch_user_code'] = change_password \
                    (cppm_connect_main, session['username'], \
                     session['new_password1'])
...
def logged_in():
    ...
    '''
    exp_days, change_pwd_next_login = exp_days_f(cppm_connect_main, session['username'])
    ...
    return render_template('logged_in.html', username=session['username'], \
        status_code=cppm_connect_main.status_code, title='Logged_in', exp_message=exp_message)
