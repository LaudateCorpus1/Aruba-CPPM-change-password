...
def test_file1_parameters(section, option):
    ...
    config = ConfigParser()
    config.read(params)
    assert config.has_option(section, option) == True
    return
...
# Сценариев пока нет
scenarios_to_check_api = [(connect_class.db_username, connect_class.db_password),
...
# @pytest.mark.parametrize("chk_name,chk_pass,api_method", scenarios_to_check_api, ids =["right_pass",\
...
scenarios_to_check_db = [(connect_class.db_username, connect_class.db_password, True),
...
# SKIP TEST if Publisher is not pinged
@pytest.mark.skipif(ping_response != 0, reason="cannot ping CPPM")
@pytest.mark.parametrize("chk_name,chk_pass,chk_flag", scenarios_to_check_db, ids=["right_pass",
                                                                                   "wrong_pass", "wrong_name"])
